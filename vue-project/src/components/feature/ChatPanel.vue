<template>
  <div class="chat-panel flex flex-col h-full relative overflow-hidden">
    ...
    <div class="flex items-center justify-between px-4 py-3 bg-[#0c1822] border-b border-white/10 relative z-30 shrink-0">
      ...
      <div class="flex items-center gap-1.5">
        ...
        <div class="relative" ref="menuRef">
          ...
          <!-- Dropdown menu (stacked cleanly above chat) -->
          <Transition
    ...
    <div class="flex-1 overflow-y-auto px-4 py-4 space-y-3 relative z-10" ref="messagesContainer" @click="showMenu = false">
      ...
      <div v-for="msg in messages" :key="msg.id"
           :class="['flex flex-col max-w-[82%]', msg.isMe ? 'ml-auto items-end' : 'mr-auto items-start']">
        ...
        <div v-else :class="['rounded-2xl px-3.5 py-2.5', msg.isMe
    ...
    <!-- Teleport Dialogs to body so they are never clipped by container -->
    <Teleport to="body">
      <!-- Confirmation Dialog -->
      <Transition
    ...
    ['kondani_chats', 'kondani_desktop_chats'].forEach(key => {
      ...
      if (stored) {
        ...
        if (Array.isArray(arr)) {
          const item = arr.find(c => String(c.id) === String(props.chatId) || (c.userId && String(c.userId) === String(getRecipientId())))
          if (item) {
            item.unread = false
            item.yourTurn = false
  ...
  try {
    ['kondani_chats', 'kondani_desktop_chats'].forEach(key => {
      ...
      if (stored) {
        ...
        if (Array.isArray(arr)) {
          const idx = arr.findIndex(c => String(c.id) === String(props.chatId) || (c.userId && String(c.userId) === String(getRecipientId())))
          if (idx !== -1) {
            const chat = arr[idx]
...
const handleNewMessage = (message) => {
  ...
  if (!isMe) {
    // When recipient responds or sends a message, previous sent messages are delivered & read
